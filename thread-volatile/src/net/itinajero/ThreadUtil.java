...
public class ThreadUtil {
   ...
   /**
    * Metodo para detener todos los hilos de tipo Process 
    */
   ...
   public static Process getThreadByName(String threadName) {     
      Set<Thread> threadSet = Thread.getAllStackTraces().keySet();
      ...
      for (Thread thread : threadArray) {        
         Class tipoThread = thread.getClass();         
         // Si es un thread del tipo que andamos buscando con el nombre
         // lo encontramos
         if (tipoThread == Process.class && thread.getName().equals(threadName)) {                             
            Process tmp = (Process) thread; // Hacemos un cast de tipo net.itinajero.Proceso            
            return tmp; // lo regresamos
         }
      }
      return null;
   }
